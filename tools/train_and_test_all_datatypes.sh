...
do
    ...
    then
        for model in ${models[@]}
        do
          echo -e ${datatype}"\t"${model}"\t"$(tp_doc_classifier.py -t ${INPUT_DIR}/${datatype} -T -c ${INPUT_DIR}/${datatype}/${model}_test.pkl -f ${FILE_TYPE} -m ${model} -n ${NGRAM_SIZE} -b ${MAX_FEATURES} -z ${TOKENIZATION})
        done
    fi
done