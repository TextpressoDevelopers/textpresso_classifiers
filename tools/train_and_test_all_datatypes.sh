...
do
    ...
    then
        for model in ${models[@]}
        do
          paste <(echo -e ${datatype}"\t"${model}) <(tp_doc_classifier.py -t ${INPUT_DIR}/${datatype} -T -c ${INPUT_DIR}/${datatype}/${model}_test.pkl -f ${FILE_TYPE} -m ${model} -n ${NGRAM_SIZE} -b ${MAX_FEATURES}) >> test_models.csv
        done
    fi
done