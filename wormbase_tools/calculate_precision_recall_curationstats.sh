...
do
    ...
    tp=0
    tmp=$(grep -o -P "SVM positive any validated true  positive</a>.*?</tr>" ${tmpfile} | grep -oP ">\K[0-9]*")
    if [[ ${tmp} != "" ]]
    then
        tp=${tmp}
    fi
    fp=0
    tmp=$(grep -o -P "SVM positive any validated false positive</a>.*?</tr>" ${tmpfile} | grep -oP ">\K[0-9]*")
    if [[ ${tmp} != "" ]]
    then
        fp=${tmp}
    fi
    tn=0
    tmp=$(grep -o -P "SVM negative validated true  negative</a>.*?</tr>" ${tmpfile} | grep -oP ">\K[0-9]*")
    if [[ ${tmp} != "" ]]
    then
        tn=${tmp}
    fi
    fn=0
    tmp=$(grep -o -P "SVM negative validated false negative</a>.*?</tr>" ${tmpfile} | grep -oP ">\K[0-9]*")
    if [[ ${tmp} != "" ]]
    then
        fn=${tmp}
    fi
